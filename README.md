...
- **飞书 ChatOps 中枢**：`feishu_gateway.py` WebSocket 长连接，底部快捷菜单 + 文本指令，按需单平台抓取或全量串行抓取（防 OOM）。
- **今日简报**：按 Notion **入库时间** 筛选过去 24 小时岗位，**不限匹配分数**。
...
└──────────────────────────────────────────────────────────────────┘
    ...
    ▼
┌──────────────────────────────────────────────────────────────────┐
│  ⑤ 飞书 ChatOps（feishu_gateway.py）                              │
│     - 今日简报：过去 24h 新入库岗位（仅时间筛选，不限分数）          │
│     - 按需抓取：9 平台单点 / 全面抓取（严格串行 subprocess）        │
│     - 深度背调：自然语言触发 OpenClaw（菜单项仅展示引导文案）        │
└──────────────────────────────────────────────────────────────────┘
    │
    ▼
