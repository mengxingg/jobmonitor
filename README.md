...
> **自动抓取 BOSS直聘 / 猎聘 / 字节跳动 / 小红书 / DeepSeek / 月之暗面 / 智谱AI / MiniMax / 腾讯 → DeepSeek AI 匹配评估 → Notion 数据库同步 → 飞书早报推送**  
...
- **飞书早报推送**：每日自动生成高分岗位早报，通过飞书机器人推送到指定会话。
- **全链路总调度**：一键触发「抓取 → 同步 → 早报推送」完整闭环。
...
┌──────────────────────────────────────────────────────────────────┐
│  ⑤ 飞书早报推送                                                   │
│     - 查询 Notion 过去 24 小时新增高分岗位（Match Score >= 80）    │
│     - AI 提炼核心匹配点                                           │
│     - 组装极客风格早报卡片，推送到飞书会话                          │
└──────────────────────────────────────────────────────────────────┘
    │
...
| **☀️ 飞书早报** | 每日自动推送高分岗位早报到飞书，AI 提炼核心匹配点 |
...
├── daily_briefing.py         # ☀️ 每日早报生成与飞书推送
├── feishu_gateway.py         # 🔌 飞书 WebSocket 网关（实时消息处理）
...
> 早报内容：查询 Notion 过去 24 小时新增的高分岗位（Match Score >= 80），AI 提炼核心匹配点，组装极客风格卡片推送。
...
`daily_briefing.py` 是项目的早报推送模块，每日自动生成高分岗位早报并推送到飞书。
...
`feishu_gateway.py` 是一个 WebSocket 监听服务，用于实时处理飞书消息。它作为常驻进程运行，接收用户在飞书中的指令并触发相应操作。

> ⚠️ **注意**：这是一个 WebSocket 常驻进程，请**手动在终端中启动**，不要使用自动化 Agent 执行。
...
# 每天早上 9:00 推送早报
0 9 * * * cd /Users/gmx/interview/job_engine && /Users/gmx/opt/anaconda3/envs/job_env/bin/python daily_briefing.py >> logs/daily_briefing.log 2>&1
```
...
| [飞书开放平台](https://open.feishu.cn/) | 早报推送 + 爬虫告警 |
