...
"""
crawler_zhipu.py — 智谱 AI (Zhipu AI) 招聘精准爬虫

使用 Playwright 访问智谱 AI 招聘页面（Moka 系统），只抓取「产品经理」相关岗位。

URL:
  https://app.mokahr.com/social-recruitment/zphz/148983?locale=zh-CN#/jobs?keyword=%E4%BA%A7%E5%93%81%E7%BB%8F%E7%90%86
...
logger = logging.getLogger("zhipu_crawler")

# ── 配置 ──
TARGET_URL = (
    "https://app.mokahr.com/social-recruitment/zphz/148983?locale=zh-CN"
...
def take_screenshot(page: Page, name: str) -> str:
    ...
    filename = f"{timestamp}_zhipu_{name}.png"
...
def click_next_page(page: Page) -> bool:
    ...
    try:
        ...
        if result == "clicked":
            ...
            logger.info(f"🔚 未找到『下一页』按钮 (JS 兜底返回: {result})")
            return False
    except Exception as e:
...
def extract_job_detail(page: Page, url: str, title: str) -> Optional[dict]:
    """访问智谱 AI 岗位详情页，提取职位描述和要求。使用降级策略：CSS 失效则 inner_text 全文。"""
    ...
    try:
        ...
        if all_job_links:
            ...
            for idx, job in enumerate(all_job_links, 1):
                # ★ 实时心跳日志：打印当前正在访问的 URL
                print(f"\n{'='*60}")
                print(f"⏱️  [{idx}/{len(all_job_links)}] 正在访问智谱 AI 详情页: {job['url']}")
            ...
            logger.info(f"\n🎉 智谱 AI 抓取完成！共 {len(all_job_details)} 条")
        else:
